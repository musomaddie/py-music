...
def create_time_signature_builder(attributes_element: Element) -> TimeSignatureBuilder:
    ...
    builder = TimeSignatureBuilder(
        int(time_element.find("beats").text),
        int(time_element.find("beat-type").text))
    builder.divisions = int(attributes_element.find("divisions").text)
