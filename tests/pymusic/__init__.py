from pymusic.original.pitch.pitchnote import PitchNote as OldPitchNote
from pymusic.pitch.pitchnote import PitchNote


def convert_into_old_note_list(note_str: str) -> list[OldPitchNote]:
    notes = []
    current_note = ""
    for letter in note_str:
        if letter in "ABCDEFG":
            if len(current_note) == 0:
                ...
                notes.append(OldPitchNote.corresponding_note_from_str(current_note))
                current_note = letter
        else:
            current_note += letter
    notes.append(OldPitchNote.corresponding_note_from_str(current_note))
